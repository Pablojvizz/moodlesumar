...
function profile_load_data(&$user) {
    ...
    if ($fields = $DB->get_records('user_info_field')) {
        foreach ($fields as $field) {
            //mbertholet - Si hay dato en el id 16 lo cargo en una variable para luego posicionar el hierselect 
            //sino cargo el dato normalmente
            if($field->id==16){
                if($DB->count_records('user_info_data', array('fieldid'=> $field->id,'userid'=>$user->id))> 0) {
                    $dato = $DB->get_record('user_info_data', array('fieldid'=> $field->id,'userid'=>$user->id));
                    $CFG->profile_field_efector = $dato->data;
                }
            }
            else{
...
function profile_save_data($usernew) {
    global $CFG, $DB;
     ...
     if (!empty($usernew->efectores['1'])) {
        $usernew->profile_field_efector = $usernew->efectores['1']; 
    } 

    //$efector = optional_param('efectores[1]', '', PARAM_RAW);
    
    //Elimino datos laborales innecesarios antes de guardar
    if ($usernew->profile_field_trabaja == 1){
        // elimino del usuario nuevo los datos que no están habilitados

        /*
        $usernew->profile_field_tconvenio = '';
        $usernew->profile_field_estOtro = '';
        $usernew->profile_field_efector = '';
        $usernew->efectores['1'] = '';
        */


        
        unset($usernew->profile_field_tconvenio);
        unset($usernew->profile_field_estOtro);
        unset($usernew->profile_field_efector);
        $DB->delete_records('user_info_data', array('userid'=>$usernew->id, 'fieldid'=> 15));
        $DB->delete_records('user_info_data', array('userid'=>$usernew->id, 'fieldid'=> 14));
        $DB->delete_records('user_info_data', array('userid'=>$usernew->id, 'fieldid'=> 16));
        
    }

    if ($usernew->profile_field_trabaja == 0 && $usernew->profile_field_tconvenio == 0){



        // elimino del usuario nuevo los datos que no están habilitados
        unset($usernew->profile_field_orgGubernamental);
        unset($usernew->profile_field_estOtro);
        unset($usernew->profile_field_orgOtro);
        $DB->delete_records('user_info_data', array('userid'=>$usernew->id, 'fieldid'=> 11));
        $DB->delete_records('user_info_data', array('userid'=>$usernew->id, 'fieldid'=> 14));
        $DB->delete_records('user_info_data', array('userid'=>$usernew->id, 'fieldid'=> 12));
    }

    if ($usernew->profile_field_trabaja == 0 && $usernew->profile_field_tconvenio == 1){
        // elimino del usuario nuevo los datos que no están habilitados
        unset($usernew->profile_field_orgGubernamental);
        unset($usernew->profile_field_efector);
        unset($usernew->profile_field_orgOtro);
    ...
    //Elimino datos laborales innecesarios antes de guardar
    if(($usernew->profile_field_cargo == 1)){
        // elimino del usuario nuevo los datos que no están habilitados
        unset($usernew->profile_field_cargoOtro);
        $DB->delete_records('user_info_data', array('userid'=>$usernew->id, 'fieldid'=> 19));                
    }
    else {
        // elimino del usuario nuevo los datos que no están habilitados
        unset($usernew->profile_field_listaprof);
        $DB->delete_records('user_info_data', array('userid'=>$usernew->id, 'fieldid'=> 18));
    }
...
function profile_display_fields($userid) {
    ...
    if ($categories = $DB->get_records('user_info_category', null, 'sortorder ASC')) {
        foreach ($categories as $category) {
            if ($fields = $DB->get_records('user_info_field', array('categoryid'=>$category->id), 'sortorder ASC')) {
                foreach ($fields as $field) {
                    //mbertholet - Si hay dato en el id 16 lo muestro en el perfil un poco trabajado
                    if($field->id==16){
                        $cuie = $DB->get_record('user_info_data', array('fieldid'=> $field->id,'userid'=>$userid));
                        if ($cuie->data != ''){
                            $efector = $DB->get_record('pn_efectores', array('cuie'=> $cuie->data));
                            $idprovincia = $DB->get_record('pn_provincia', array('id'=> $efector->id_provincia));
                            print_row(format_string($field->name.':'), $efector->nombre . ' ('. $idprovincia->nombre . ')');
                        }
                    }
                    else{
                        require_once($CFG->dirroot.'/user/profile/field/'.$field->datatype.'/field.class.php');
...
function profile_signup_fields(&$mform) {
    ...
    if ( $fields = $DB->get_records_sql($sql)) {
        ...
        foreach ($fields as $field) {
            if ($field->categoryname != 'personalizados') {
