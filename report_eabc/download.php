...
$query = "SELECT DISTINCT provincia_de_residencia , tipo_y_lugar_de_trabajo, establecimiento, tipo_de_organismo_y_organismo,tipo_de_función, función_que_desempeña, 
          ...
          WHERE userid not in (29,109) and ('".$USER->username."' = provincia_user or '".$USER->username."' in ('eabc', 'maria','admin','gestionpn') ) and ( Nivel_de_avance like '%".$year."%' or fecha_matriculacion like '%".$year."%' ) order by nombres asc , Curso asc , Nivel_de_avance desc" or die("Error in the consult.." . mysqli_error($link));
//echo $query ; exit;          
